...
def calculate_current_rate(start_rate, target_rate, transition_duration, elapsed_time):
    ...
    randomSpice = random.random() * 0.001

    # Calculate the rate of change
    rate_of_change = math.log(target_rate / start_rate) / transition_duration  # We add a random spice to the rate of change to simulate real-world scenarios
    
    # Calculate the current rate based on elapsed time
    current_rate = start_rate * math.exp(rate_of_change * (elapsed_time + randomSpice))
...
def generate_change_intervals(total_duration_sec, window_size=10):
    total_windows = total_duration_sec // window_size
...
def simulate_window_acceleration( start_time,change_windows, transition_duration=6, stable_rate_ms=400, initial_stable_period_sec = 10, change_window_size=10, max_acceleration_rate_ms=100, max_deceleration_rate_ms=500):
    ...
    window_start_boundary = 1
    window_end_boundary = change_window_size - 2 if (change_window_size - 2) != 0 else change_window_size  
    ...
    if current_window == 0:
        ...
        rand_start = (current_window * change_window_size) + random.randint(window_start_boundary, window_end_boundary)
